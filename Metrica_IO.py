...
def to_single_playing_direction(home, away, events):
    '''
    Flip coordinates in the second half so that each team always shoots in the same direction throughout the match.
    In a [0, 1] coordinate system, we mirror the coordinates around 0.5.
    '''
    for team in [home, away, events]:
        ...
        # Get columns that end with 'x' or 'y' (indicating coordinates)
        columns = [c for c in team.columns if c[-1].lower() in ['x','y']]
        
        # Flip coordinates around 0.5 for the second half only
        team.loc[second_half_idx:, columns] = 1 - team.loc[second_half_idx:, columns]
